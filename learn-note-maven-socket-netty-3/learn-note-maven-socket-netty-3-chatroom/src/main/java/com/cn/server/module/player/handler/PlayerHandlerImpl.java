...
public class PlayerHandlerImpl implements PlayerHandler{
	...
	public Result<PlayerResponse> registerAndLogin(Session session, byte[] data) {
		...
		try {
			...
			player.setPlayerName(registerRequest.getPlayerName());
			player.setPassward(registerRequest.getPassward());
			result = playerService.registerAndLogin(session, player);
	...
	public Result<PlayerResponse> login(Session session, byte[] data) {
		...
		try {
			...
			//执行业务
			Player player = new Player();
			player.setPlayerName(loginRequest.getPlayerName());
			player.setPassward(loginRequest.getPassward());
			result = playerService.login(session, player);
		} catch (ErrorCodeException e) {
			return Result.ERROR(e.getErrorCode());
		} catch (Exception e) {
			e.printStackTrace();
